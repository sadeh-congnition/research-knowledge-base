...
def command():
    ...
    for node in nodes:
        ...
        if "Transcript:" in node.content:
            click.echo(f"Skipping '{node.title}' - transcript already appears to be present.")
            continue
        ...
        if match:
            ...
            click.echo(f"Fetching transcript for video ID '{video_id}' (Node: '{node.title}')...")
            ...
            try:
                ...
                text_formatted = "\n".join([x['text'] if isinstance(x, dict) else x.text for x in fetched_transcript])
                
                node.content += f"\n\nTranscript:\n{text_formatted}"
                node.save()
                click.secho(f"Successfully appended transcript to '{node.title}'.", fg="green")
                count += 1
                ...
                click.secho(f"Failed to fetch transcript for '{node.title}': {e}", fg="red")
        else:
