...
class KnowledgeBaseTests(TestCase):
    def setUp(self):
        ...
        self.node1 = Node.objects.create(project=self.project, title="Node 1", content="Content 1")
        self.node2 = Node.objects.create(project=self.project, title="Node 2", content="Content 2")
    ...
    def test_project_create_view(self):
        response = self.client.post(reverse('project_create'), {'name': 'New Project', 'description': 'New Desc'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_node_create_view(self):
        response = self.client.post(reverse('node_create', args=[self.project.pk]), {
            'title': 'New Node',
    ...
    def test_node_update_view(self):
        response = self.client.post(reverse('node_update', args=[self.node1.pk]), {
    ...
    def test_node_add_link_view(self):
        response = self.client.post(reverse('node_add_link', args=[self.node1.pk]), {
